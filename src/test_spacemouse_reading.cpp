...
void testSpacemouseReadingThread(const std::string& hostname) { 
  try {
    ...
    robot.control([&timeSincePrint, &shouldPrint, &initialized, &current_pose, &currentInput, &goalPosition, &difference](const franka::RobotState& robot_state, franka::Duration period) -> franka::CartesianPose {
      ...
      return franka::CartesianPose(robot_state.O_T_EE); // return init pose so robot is stationary for test
    });
